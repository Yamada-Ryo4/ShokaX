...
> "生活总是在你准备放弃的时候，给你一点点甜头，让你觉得还能再坚持一下。"
...
如果是在以前，我可能又要在 Docker 报错日志里淹没了。但这次在本地环境下，我用到了 [yetone](https://x.com/yetone) 大佬开发的 [Alma](https://alma.now/) AI Agent 工具。我和 Alma 像剥洋葱一样，把主题的 CSS 样式和 JS 脚本一层层拆开：

-   **关于抖动**：我们发现是侧边栏的固定逻辑和页面高度在“互相伤害”，于是直接用 **CSS Sticky** 强行劝架。
-   **关于打赏**：我们发现打赏码的配置合并逻辑有坑，于是干脆写了个脚本在构建前从内存里直接把不需要的项“硬删除”了。
...
-   **稳定性**：经过持久化补丁修复，哪怕内容再少，也不会再有任何抖动。
